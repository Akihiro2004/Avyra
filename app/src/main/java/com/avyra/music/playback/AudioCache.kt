 ...
 * original `avyra://watch?v=<id>` request, and a cache hit never resolves a
 * URL at all.
 */
@UnstableApi
object AudioCache {
     ...
     * [StreamResolver][com.avyra.music.data.innertube.StreamResolver]'s
     ...
     * `avyra://watch?v=…` URI it builds for itself carries an id and nothing
     * else. It rides along from the queue instead — see
     * [PlaybackService.prefetchAround][com.avyra.music.playback.PlaybackService].
     */
     ...
     * [PlaybackService][com.avyra.music.playback.PlaybackService]'s audition.
    ...
    fun requestAnalysisHead(uri: Uri) {
        ...
        scope.launch {
            try {
                ...
                fetch(
                    cacheKey = videoId,
                    uri = Uri.parse("avyra://watch?v=$videoId"),
                    position = 0,
                    length = want,
                    pinKey = true,
                )
    ...
    private suspend fun fetch(videoId: String, position: Long, length: Long) =
        fetch(videoId, Uri.parse("avyra://watch?v=$videoId"), position, length)
