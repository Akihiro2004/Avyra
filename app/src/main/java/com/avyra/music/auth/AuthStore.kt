...
class AuthStore(context: Context) {
    ...
    private val prefs: SharedPreferences = runCatching {
        LegacyDataMigration.encryptedPreferences(
            context = context,
            currentEncryptedName = "avyra_auth",
            legacyEncryptedName = "bitchord_auth",
            currentPlainName = "avyra_auth_plain",
            legacyPlainName = "bitchord_auth_plain",
        )
    }.getOrElse {
        Log.w("Avyra", "EncryptedSharedPreferences unavailable, falling back: ${it.message}")
        LegacyDataMigration.plainPreferences(
            context = context,
            currentName = "avyra_auth_plain",
            legacyName = "bitchord_auth_plain",
        )
    }
