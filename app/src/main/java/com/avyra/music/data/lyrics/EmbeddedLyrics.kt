package com.avyra.music.data.lyrics
 ...
 *
 *  - `AVYRA_LYRICS`, this app's own, holding the "enhanced" A2 form with the
...
object EmbeddedLyrics {
    ...
    private val WORD_LYRICS_FIELDS = listOf(WORD_LYRICS_FIELD, LEGACY_WORD_LYRICS_FIELD)
    ...
    private fun mp4(bytes: ByteArray): String? {
        ...
        return ilstText(bytes, moov.first, end, freeform = true)
            ?: ilstText(bytes, moov.first, end, freeform = true, legacy = true)
    ...
    private fun ilstText(
        ...
        val marker = when {
            !freeform -> LYR_ATOM
            legacy -> LEGACY_WORD_LYRICS_FIELD.toByteArray(Charsets.UTF_8)
            else -> WORD_LYRICS_FIELD.toByteArray(Charsets.UTF_8)
        }
        var at = from
        while (true) {
    ...
    private fun matroska(bytes: ByteArray): String? {
        ...
        for (name in WORD_LYRICS_FIELDS + "LYRICS") {
            ...
            while (true) {
                ...
                if (name in WORD_LYRICS_FIELDS) return value
