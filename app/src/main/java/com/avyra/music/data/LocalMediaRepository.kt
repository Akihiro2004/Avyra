...
object LocalMediaRepository {
    ...
    /**
     * Retrieves songs from `Music/Avyra` and the legacy `Music/Avyra`
     * directory, combining app downloads with local audio files in either folder.
     *
    ...
    suspend fun getDownloadedSongs(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        runCatching {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                DownloadStore.READABLE_FOLDERS.forEach { folderName ->
                    val folder = File(
        ...
        }.onFailure { Log.w(TAG, "Failed scanning Avyra download directories: ${it.message}") }
    ...
    suspend fun getLocalMusic(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        // whatever this app has downloaded into Music/Avyra alongside
    ...
    /**
     * MediaStore's music flag is advisory and often includes notifications,
     * voice notes and recorder output. This second vendor-independent gate
     * keeps those rows out while retaining long-form WAV and lossless music.
     */
    internal fun isEligibleLocalMusic(
        durationMs: Long,
        displayName: String,
        path: String?,
    ): Boolean {
    ...
    private fun String.removeAudioExtension(): String {
        ...
        return if (extension in localMusicExtensions) {
