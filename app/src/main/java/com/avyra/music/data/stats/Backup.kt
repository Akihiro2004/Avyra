package com.avyra.music.data.stats
...
object Backup {
    ...
    fun suggestedName(): String =
        "avyra-backup-${DateTimeFormatter.ofPattern("yyyy-MM-dd").format(
    ...
    suspend fun importFrom(context: Context, source: Uri): Result<Summary> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val file = runCatching { json.decodeFromString(BackupFile.serializer(), text) }
                .getOrElse { error("That doesn't look like a Avyra backup") }
            require(file.app in SUPPORTED_APP_TAGS) { "That backup is from another app" }
            require(file.version <= SCHEMA_VERSION) {
                "That backup was written by a newer version of Avyra"
            }
    ...
    private const val APP_TAG = "avyra"
    private const val LEGACY_APP_TAG = "bitchord"
    private val SUPPORTED_APP_TAGS = setOf(APP_TAG, LEGACY_APP_TAG)
