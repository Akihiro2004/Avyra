...
object AppSettings {
    ...
    // [SourceResolver.requestForNow][com.avyra.music.data.sources.SourceResolver.requestForNow],
    ...
    /**
     * Whether Avyra's own ten-band EQ is shaping the output.
     *
     * Separate from the band gains so that switching it off keeps the curve
     * the listener spent time on: an EQ that forgot its settings the moment it
     * was bypassed would make A/B-ing it against flat impossible, which is the
     * one thing anyone actually does with the switch.
     */
    /**
     * Whether the listener has been through onboarding.
     *
     * Its own flag rather than being inferred from anything else. "Has a queue"
     * or "is signed in" both look like a returning listener and are both wrong:
     * someone can finish onboarding, decline the sign-in, play nothing, and
     * close the app — and showing them the welcome again on next launch would
     ...
     * all. See [CanvasRepository][com.avyra.music.data.canvas.CanvasRepository].
     */
     ...
     * — see [CanvasCache][com.avyra.music.data.canvas.CanvasCache] for why
    ...
    /**
     * The order [lyricsSources] are asked in — see [LyricsRepository][com.avyra.music.data.lyrics.LyricsRepository]:
     ...
     * [ArtistFacts][com.avyra.music.data.stats.ArtistFacts]. On by default,
     ...
     * [PlaybackService][com.avyra.music.playback.PlaybackService] can pick
     ...
     *
     * The one caller is an import ([Backup][com.avyra.music.data.stats.Backup]),
     ...
     *
     * Avyra ships sideloaded (see [com.avyra.music.data.AppUpdateChecker]),
     ...
     *
     * [Downloads][com.avyra.music.download.Downloads] keeps its record of
