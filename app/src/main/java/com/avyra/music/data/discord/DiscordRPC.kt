package com.avyra.music.data.discord
 ...
 *
 * ```
 *   Listening to Avyra             <- activityName, or the app's own name
 *   ┌────┐  Song title             <- details
 *   │art │  Artist                 <- state
...
class DiscordRPC(
    ...
    suspend fun updateSong(
        ...
        check(isConfigured) { "Avyra Discord application ID is not configured" }
     ...
     * a side-by-side dev install should still look like Avyra to everyone
     * else on Discord.
     */
    ...
    companion object {
         ...
         *
         * Discord needs this for the endpoint that mirrors artwork onto its CDN
         * and for activity buttons. It must belong to the Avyra maintainer.
         *
         * It does *not* decide the name shown on the profile — that is
         * `name` in the activity payload, which [appName] fills in. Configure
         * `AVYRA_DISCORD_APPLICATION_ID` in `local.properties`.
         */
        private val APPLICATION_ID = BuildConfig.DISCORD_APPLICATION_ID
