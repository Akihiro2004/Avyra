 ...
 * Avyra to access all device logs?"* — which appears whenever the process
 ...
 * to logcat as well, so `adb logcat -s Avyra` is unchanged.
 */
object TrackLog {
    ...
    private fun header(
        ...
        appendLine("Avyra log — ${song.title} — ${song.artist}")
